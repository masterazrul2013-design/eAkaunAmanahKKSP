// Universal Modal Component with Pinned Header & Sticky Footer Bar
export function renderModal({ title, bodyHtml, footerButtons = [], onClose }) {
  ...
  backdrop.className = "fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-900/65 backdrop-blur-sm animate-fade-in";

  backdrop.innerHTML = `
    <div class="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-2xl max-h-[92vh] flex flex-col overflow-hidden my-auto animate-fade-in" onclick="event.stopPropagation()">
      <!-- Modal Header Bar (Pinned at Top) -->
      <div class="px-5 py-3 bg-slate-900 text-white flex items-center justify-between border-b border-slate-800 modal-header-bar shrink-0">
        <h3 class="text-xs sm:text-sm font-bold tracking-tight text-white flex items-center gap-2">
          <span>📋</span> ${title}
        </h3>
        <div class="flex items-center gap-2">
          <button onclick="window.print()" class="px-3 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold transition flex items-center gap-1 shadow-sm no-print">
            <span>🖨️</span> Cetak Sekarang
          </button>
          <button id="modal-close-btn" class="w-7 h-7 rounded-lg hover:bg-white/10 flex items-center justify-center text-slate-400 hover:text-white transition font-bold">
            ✕
          </button>
        </div>
      </div>

      <!-- Modal Body Content (Scrollable Middle with Custom Scrollbar) -->
      <div class="p-4 overflow-y-auto max-h-[72vh] flex-1 space-y-4 text-slate-700 text-xs modal-body-content" style="scroll-behavior: smooth;">
        ${bodyHtml}
      </div>

      <!-- Modal Footer Bar (Pinned at Bottom) -->
      <div class="px-5 py-3 bg-slate-100 border-t border-slate-300 flex items-center justify-end gap-3 modal-footer-bar shrink-0 z-10" id="modal-footer-container">
      </div>
    </div>
  `;
  ...
  footerButtons.forEach((btn) => {
    ...
    buttonEl.className = btn.className || "px-4 py-2 rounded-lg text-xs font-semibold transition";
